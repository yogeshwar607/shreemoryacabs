# Google My Business Optimization Checklist - GreenEVCabs

## 🎯 **Goal:** Dominate local search for "electric cab pune mumbai airport" and related keywords

---

## ✅ **Phase 1: Profile Completion (Week 1 - Day 1-3)**

### **Basic Information (Must Complete 100%)**
- [ ] **Business Name:** GreenEVCabs - Electric Cab Service
- [ ] **Primary Category:** Taxi Service
- [ ] **Additional Categories:**
  - [ ] Airport Shuttle Service
  - [ ] Electric Vehicle Rental Service
  - [ ] Transportation Service
  - [ ] Cab Service
- [ ] **Business Description (750 characters):**
  ```
  GreenEVCabs is Maharashtra's premier 100% electric cab service specializing in airport transfers and outstation trips. We operate premium electric vehicles (Kia Carens EV & BYD E-MAX 7) for Pune to Mumbai Airport, Mumbai to Pune, and all major Maharashtra routes. Our eco-friendly airport transfer service features zero surge pricing (₹4,500 fixed), real-time flight tracking, 24/7 availability, meet & greet with name board, and corporate packages with GST invoices. Perfect for business travelers, families, and IT professionals from Hinjewadi, Wakad, and other Pune areas. Experience zero-emission, comfortable travel with professional drivers. Book instantly via WhatsApp or call for airport pickups, outstation trips, and corporate transportation.
  ```

### **Contact Information**
- [ ] **Phone:** +91 74982 64215 (ensure it's verified)
- [ ] **Website:** https://greenevcabs.com
- [ ] **Booking URL (if different):** https://wa.me/917498264215
- [ ] **WhatsApp:** +91 74982 64215

### **Business Hours**
- [ ] Set to **24/7 Open** or **Open 24 Hours**
- [ ] Monday: Open 24 hours
- [ ] Tuesday: Open 24 hours
- [ ] Wednesday: Open 24 hours
- [ ] Thursday: Open 24 hours
- [ ] Friday: Open 24 hours
- [ ] Saturday: Open 24 hours
- [ ] Sunday: Open 24 hours

### **Service Areas (Define Precisely)**
- [ ] **Primary:** Pune
- [ ] **Secondary Areas:**
  - [ ] Pimpri-Chinchwad
  - [ ] Mumbai
  - [ ] Thane
  - [ ] Navi Mumbai
  - [ ] Lonavala
  - [ ] Shirdi
  - [ ] Nashik
  - [ ] Mahabaleshwar

### **Attributes (Select All Applicable)**
- [ ] Wheelchair accessible
- [ ] Online booking available
- [ ] Credit card accepted
- [ ] Debit card accepted
- [ ] UPI accepted
- [ ] Family-friendly
- [ ] LGBTQ+ friendly
- [ ] Women-led business (if applicable)

---

## 📸 **Phase 2: Photo Upload Strategy (Week 1 - Day 3-7)**

### **Target: 100+ Photos Total**

#### **Vehicle Photos (30 photos minimum)**
- [ ] Kia Carens EV - Exterior (5 angles: front, rear, side, 3/4, full body)
- [ ] Kia Carens EV - Interior (10 photos: dashboard, seats, boot, features, charging port)
- [ ] BYD E-MAX 7 - Exterior (5 angles)
- [ ] BYD E-MAX 7 - Interior (10 photos)
- [ ] Clean vehicles ready for pickup
- [ ] Luggage loaded in boot
- [ ] Charging station photos
- [ ] Driver assisting with luggage

#### **Airport Service Photos (20 photos minimum)**
- [ ] Driver with name board at airport
- [ ] Terminal 1 pickup point
- [ ] Terminal 2 pickup point
- [ ] Customers at Mumbai Airport
- [ ] Customers at Pune Airport
- [ ] Meet & greet at arrival gate
- [ ] Luggage loading at airport
- [ ] Professional driver in uniform at airport
- [ ] Vehicle at airport drop-off area

#### **Route & Landmark Photos (15 photos minimum)**
- [ ] Mumbai-Pune Expressway drive
- [ ] Lonavala tunnel
- [ ] Pune IT parks (Hinjewadi, Magarpatta)
- [ ] Mumbai landmarks
- [ ] Pune landmarks
- [ ] Shirdi temple (for Shirdi route)

#### **Customer Photos (20 photos minimum)**
- [ ] Happy customers with driver (10 different groups)
- [ ] Families in vehicle
- [ ] Business travelers in vehicle
- [ ] Customer testimonials with photos
- [ ] Corporate clients

#### **Team & Operations (10 photos minimum)**
- [ ] Driver profile photos (5 drivers)
- [ ] Team meeting/briefing
- [ ] Vehicle maintenance
- [ ] Office/operations center
- [ ] Safety equipment

#### **Branding & Certifications (5 photos)**
- [ ] GreenEVCabs logo
- [ ] Vehicle branding
- [ ] Green certifications (if any)
- [ ] Awards/recognitions
- [ ] Zero emission badge/certification

### **Photo Optimization Rules**
- **Resolution:** Minimum 720px width/height
- **Format:** JPG preferred (smaller file size)
- **File Names:** Include keywords
  - `greenevcabs-electric-cab-mumbai-airport.jpg`
  - `pune-to-mumbai-airport-electric-taxi.jpg`
  - `kia-carens-ev-interior-airport-transfer.jpg`
- **Alt Text:** (Add when uploading via GMB dashboard)
  - "GreenEVCabs electric cab at Mumbai Airport Terminal 2"
  - "Kia Carens EV interior - Premium airport transfer service"

---

## 🎥 **Phase 3: Video Content (Week 2)**

### **Videos to Upload (5-10 videos, 1-3 minutes each)**

1. **"How Airport Pickup Works"** (2 minutes)
   - Show name board service
   - Driver at terminal
   - Customer meeting
   - Luggage loading
   - Journey starts

2. **"Inside Our Electric Cabs"** (2 minutes)
   - Vehicle tour
   - Interior features
   - Comfort highlights
   - Space demonstration

3. **"Pune to Mumbai Airport Journey"** (3 minutes fast-forward)
   - Pickup in Pune
   - Expressway drive
   - Mumbai Airport arrival
   - Drop-off

4. **"Customer Testimonials"** (5 x 30-second clips)
   - Real customers sharing experience
   - Focus on airport transfers

5. **"Meet Our Drivers"** (1 minute)
   - Professional introduction
   - Driver talking about service quality

### **Video Optimization**
- **Title Format:** "GreenEVCabs - [Topic] | Electric Cab Pune Mumbai Airport"
- **Description:** Include keywords & link to website
- **Thumbnail:** Clear, high-quality image with text overlay

---

## 🔍 **Phase 4: Google Posts Strategy (Ongoing - 3 posts/week)**

### **Post Types & Schedule**

#### **Monday: Pricing/Offer Post**
```
✈️ Pune to Mumbai Airport at ₹4,500!

Zero surge pricing | 24/7 service | Flight tracking included

📍 Book now: [Link]
📞 +91 74982 64215

#ElectricCab #MumbaiAirport #PuneToMumbai
```

#### **Wednesday: Customer Testimonial**
```
⭐ "Smooth ride, professional driver, and eco-friendly!"
- Rahul S., Hinjewadi

5-star electric cab service for airport transfers.

Book your next trip: [Link]
```

#### **Friday: Service Feature / Tip**
```
💡 Did you know?

Our electric cabs track your flight in real-time. If your flight is delayed, we automatically adjust pickup time - at no extra cost!

Experience hassle-free airport pickups: [Link]
```

### **Post Guidelines**
- **Length:** 100-300 words
- **CTA:** Always include booking link or phone number
- **Images:** Attach relevant photo with each post
- **Frequency:** Minimum 3 posts/week
- **Best Times:** 9 AM, 1 PM, 6 PM (when people book cabs)

---

## ❓ **Phase 5: Q&A Optimization (Week 2)**

### **Pre-populate 20 Questions**

1. **Q:** How much does cab from Pune to Mumbai Airport cost?
   **A:** Our electric cab from Pune to Mumbai Airport costs ₹4,500 (Kia Carens) with zero surge pricing. This includes all tolls, parking, and no hidden charges. Book via WhatsApp: +91 74982 64215

2. **Q:** Do you provide 24/7 service?
   **A:** Yes! We operate 24/7 including early morning airport drops (3 AM, 4 AM) and late night pickups. Book anytime: https://greenevcabs.com/routes/pune-to-mumbai-airport

3. **Q:** How early should I book?
   **A:** We recommend booking 24 hours in advance for guaranteed availability, especially for early morning flights. Last-minute bookings accepted subject to availability.

4. **Q:** Do you track flights for airport pickup?
   **A:** Yes! We track all flights in real-time. If delayed, our system automatically adjusts pickup time with zero waiting charges. Your driver will be there when you land.

5. **Q:** Is GST invoice provided?
   **A:** Yes, we provide GST invoices for all bookings. Perfect for corporate expense reimbursement. Corporate packages available with monthly billing.

6. **Q:** Which electric vehicles do you use?
   **A:** We operate Kia Carens EV (6-7 seater) and BYD E-MAX 7 (7 seater) - both premium electric SUVs with 400+ km range. Comfortable, quiet, zero emissions.

7. **Q:** Can I book for one-way trip only?
   **A:** Yes, we provide one-way trips from Pune to Mumbai Airport (₹4,500) or Mumbai Airport to Pune (₹4,500). Round trip packages available with 10% discount.

8. **Q:** Do you pick up from Hinjewadi to Mumbai Airport?
   **A:** Yes! We provide pickup from all Hinjewadi locations (Phase 1, 2, 3 IT parks) to Mumbai Airport at ₹4,500. Corporate packages available: https://greenevcabs.com/routes/hinjewadi-to-mumbai-airport

9. **Q:** What about luggage space?
   **A:** Our electric cabs accommodate 4-5 large suitcases plus cabin bags. No extra charge for luggage. Perfect for family/group airport transfers.

10. **Q:** How do I book?
    **A:** Three easy ways: 1) WhatsApp: +91 74982 64215, 2) Call: +91 74982 64215, 3) Website: https://greenevcabs.com - Instant confirmation!

11. **Q:** Are tolls included in price?
    **A:** Yes, all tolls on Mumbai-Pune Expressway (₹440) are included in our ₹4,500 fare. No additional charges.

12. **Q:** Do you serve Navi Mumbai Airport?
    **A:** Yes! We provide electric cab service to Navi Mumbai International Airport (NMIA) from Pune at ₹4,800. Visit: https://greenevcabs.com/airport

13. **Q:** Can I make stops during journey?
    **A:** Yes, reasonable stops for refreshments/restroom breaks are allowed. Please inform during booking if you plan specific stops.

14. **Q:** What is cancellation policy?
    **A:** Free cancellation up to 6 hours before pickup. Cancellations within 6 hours: 50% charge. We try to be flexible - contact us ASAP if plans change.

15. **Q:** Do you drop at both Terminal 1 and Terminal 2?
    **A:** Yes, we provide drop service to both Terminal 1 (Domestic) and Terminal 2 (International) at Mumbai Airport. Just inform us your airline/terminal during booking.

16. **Q:** How long does Pune to Mumbai Airport take?
    **A:** Journey takes 2.5-3 hours via Expressway depending on traffic. We recommend leaving 5-6 hours before your flight for comfortable check-in.

17. **Q:** Is your cab really 100% electric?
    **A:** Yes! Our entire fleet is 100% electric - zero emissions, zero petrol/diesel. You can verify by seeing our Kia Carens EV and BYD E-MAX 7 vehicles.

18. **Q:** Do you provide monthly packages?
    **A:** Yes! We offer monthly packages for frequent travelers and corporates with volume discounts. Contact us for custom pricing: +91 74982 64215

19. **Q:** How will I find driver at Mumbai Airport?
    **A:** For pickups, driver waits at designated point with name board displaying your name. You'll receive driver photo, name, phone, and vehicle details via WhatsApp 2 hours before landing.

20. **Q:** Can I book for Wakad/Kothrud/Hadapsar to Mumbai Airport?
    **A:** Yes! We pick up from ALL Pune locations to Mumbai Airport at ₹4,500 flat rate. Wakad, Kothrud, Hadapsar, Viman Nagar, Baner, Aundh - all covered.

### **Q&A Management**
- Monitor for new questions weekly
- Respond within 24 hours
- Use keywords naturally in answers
- Include booking links where relevant

---

## ⭐ **Phase 6: Review Generation Strategy (Ongoing)**

### **Goal: 100 Reviews in 3 Months (1 review/day)**

#### **Week 1-2: Quick Win (Target: 20 reviews)**
- [ ] Contact last 50 customers via WhatsApp
- [ ] Message template:
  ```
  Hi [Name], thank you for choosing GreenEVCabs! We hope you enjoyed your eco-friendly ride to Mumbai Airport. Would you mind sharing your experience on Google? It helps us serve you better!

  [Google Review Link]

  As a thank you, get ₹200 off your next booking! 🎁
  ```
- [ ] Call top 10 corporate clients personally
- [ ] Offer ₹200 discount for reviews

#### **Ongoing Review Process**
1. **After Trip Completion:**
   - Send WhatsApp message within 30 minutes
   - Include direct Google review link
   - Make it easy with 1-click access

2. **Follow-up (Day 3):**
   - If no review, send gentle reminder
   - "We'd love to hear your feedback!"

3. **Incentive Structure:**
   - ₹200 off next booking for Google review
   - Mention in monthly newsletter
   - Feature best reviews on website

4. **In-Vehicle Prompt:**
   - QR code sticker inside cab linking to review page
   - Driver reminder (polite, not pushy)
   - "If you enjoyed the ride, please review us!"

#### **Review Response Template**

**For 5-Star Reviews:**
```
Thank you so much, [Name]! 🌟 We're thrilled you enjoyed your electric cab ride from Pune to Mumbai Airport. We look forward to serving you again on your next eco-friendly journey! Safe travels! 🚗⚡

Book again: https://greenevcabs.com
```

**For 4-Star Reviews:**
```
Thank you for your feedback, [Name]! We're glad you chose GreenEVCabs for your airport transfer. We'd love to know how we can make your next experience 5-star. Feel free to reach us at +91 74982 64215. See you soon! 🌱
```

**For 3-Star or Lower:**
```
Thank you for bringing this to our attention, [Name]. We sincerely apologize for falling short of expectations. I'd like to personally discuss this with you to make it right. Please call/WhatsApp me at +91 74982 64215 at your convenience. We value your patronage.

- Team GreenEVCabs
```

---

## 🔗 **Phase 7: Services Section Optimization**

### **Services to Add (15-20 services)**

1. **Pune to Mumbai Airport Electric Cab**
   - Description: "Zero surge ₹4,500 | Flight tracking | 24/7"
   - Link: https://greenevcabs.com/routes/pune-to-mumbai-airport

2. **Mumbai Airport to Pune Cab Pickup**
   - Description: "Meet & greet | Name board | Flight delay adjustment"
   - Link: https://greenevcabs.com/routes/mumbai-airport-to-pune

3. **Hinjewadi to Mumbai Airport Electric Taxi**
   - Description: "Corporate packages | GST invoice | IT Park pickup"
   - Link: https://greenevcabs.com/routes/hinjewadi-to-mumbai-airport

4. **Pune to Mumbai Electric Cab**
   - Description: "Expressway route | 148 km | ₹4,500"
   - Link: https://greenevcabs.com/routes/pune-mumbai

5. **Mumbai to Pune Electric Cab**
   - Description: "Return trips | Zero emissions | Premium EVs"
   - Link: https://greenevcabs.com/routes/mumbai-pune

6. **Electric Cab Airport Transfer Service**
   - Description: "All Maharashtra airports | Zero surge | 24/7"
   - Link: https://greenevcabs.com/airport

7. **Pune to Shirdi Electric Cab**
   - Description: "Temple visits | Spacious 7-seater | ₹5,500"
   - Link: https://greenevcabs.com/routes/pune-shirdi

8. **Pune to Lonavala Electric Cab**
   - Description: "Weekend getaways | Hill station | ₹2,800"
   - Link: https://greenevcabs.com/routes/pune-lonavala

9. **Corporate Electric Cab Service**
   - Description: "Monthly packages | Employee transport | GST invoices"
   - Link: https://greenevcabs.com/corporate

10. **Electric Cab Outstation Service**
    - Description: "Maharashtra-wide | Zero emissions | Premium comfort"
    - Link: https://greenevcabs.com/routes

11. **Wakad to Mumbai Airport Cab**
    - Description: "Fixed ₹4,500 | No surge | Early morning available"

12. **Kothrud to Mumbai Airport Cab**
    - Description: "Central Pune pickup | 24/7 service | ₹4,500"

13. **Hadapsar to Mumbai Airport Cab**
    - Description: "Magarpatta area | Corporate friendly | ₹4,500"

14. **Viman Nagar to Mumbai Airport Cab**
    - Description: "Near Pune Airport | Quick expressway access | ₹4,400"

15. **Baner to Mumbai Airport Electric Cab**
    - Description: "West Pune | IT professionals | ₹4,500"

---

## 📊 **Phase 8: Insights Monitoring & Optimization**

### **Weekly Metrics to Track**

#### **Search Metrics**
- [ ] Total searches for "electric cab"
- [ ] Total searches for "airport transfer"
- [ ] Total searches for "pune mumbai"
- [ ] Direct searches (people searching for "GreenEVCabs")
- [ ] Discovery searches (people finding you organically)

#### **Action Metrics**
- [ ] Website clicks
- [ ] Phone calls
- [ ] Direction requests
- [ ] Photo views
- [ ] Photo count (competitor comparison)

#### **Benchmark Goals (Month 1)**
- **Total searches:** 500-1,000/month
- **Website clicks:** 100-200/month (10-20% CTR)
- **Phone calls:** 50-100/month (5-10% call rate)
- **Direction requests:** 20-40/month
- **Photo views:** 200-500/month

#### **Optimization Actions Based on Data**
- If low searches → Improve keyword usage in description, posts, Q&A
- If low clicks → Update photos, improve description, add more posts
- If low calls → Make phone number more prominent, add call-to-action
- If low photo views → Upload more high-quality photos with keywords

---

## 🎯 **Phase 9: Advanced Optimization (Month 2)**

### **Competitor Analysis**
- [ ] Find top 3 electric cab competitors in Pune
- [ ] Analyze their GMB profiles
- [ ] Note: photos count, review count, response rate
- [ ] Identify gaps we can fill
- [ ] Create better content than competitors

### **Local SEO Citations**
- [ ] Ensure NAP (Name, Address, Phone) consistency across:
  - [ ] Google My Business
  - [ ] Website
  - [ ] Facebook
  - [ ] Instagram
  - [ ] JustDial
  - [ ] Sulekha
  - [ ] IndiaMART

### **Google My Business Website**
- [ ] If no website, create free GMB website
- [ ] Add airport transfer keywords
- [ ] Include pricing information
- [ ] Add booking button

### **Special Offers**
- [ ] Create "First Ride" offer: ₹3,999 Pune-Mumbai Airport
- [ ] "Round Trip Special": ₹8,100 (save ₹900)
- [ ] "Corporate Welcome": 15% off first month
- [ ] Update offers every 2 weeks to keep profile fresh

---

## ✅ **Phase 10: Weekly Checklist (Ongoing Maintenance)**

### **Monday**
- [ ] Post: Pricing/Offer update
- [ ] Check & respond to new reviews (target < 24 hours)
- [ ] Review last week's insights
- [ ] Identify improvement areas

### **Wednesday**
- [ ] Post: Customer testimonial or success story
- [ ] Check for new Q&A questions
- [ ] Answer any pending questions
- [ ] Upload 2-3 new photos if available

### **Friday**
- [ ] Post: Service feature or travel tip
- [ ] Review weekly performance metrics
- [ ] Plan next week's content
- [ ] Send review request to last week's customers

### **Monthly (1st of month)**
- [ ] Upload 10-15 new photos
- [ ] Create 1 new video
- [ ] Update business description (if needed)
- [ ] Review & improve low-performing services
- [ ] Analyze competitor updates
- [ ] Report on KPIs: searches, clicks, calls, reviews

---

## 🎖️ **Success Metrics**

### **Month 1 Goals**
- ✅ 100% profile completion
- ✅ 50+ high-quality photos uploaded
- ✅ 20+ reviews collected
- ✅ 12+ posts published
- ✅ 20 Q&As pre-populated
- ✅ 15 services added

### **Month 3 Goals**
- ✅ 100+ photos
- ✅ 75+ reviews (4.8+ rating)
- ✅ 36+ posts
- ✅ 1,000+ monthly searches
- ✅ 200+ website clicks/month
- ✅ 100+ phone calls/month

### **Month 6 Goals**
- ✅ 150+ photos
- ✅ 150+ reviews (4.9+ rating)
- ✅ 2,000+ monthly searches
- ✅ Rank in top 3 for "electric cab pune"
- ✅ Rank in top 3 for "pune mumbai airport cab"
- ✅ 50%+ of bookings from organic GMB search

---

## 🚀 **Quick Start Action Plan (This Week)**

### **Day 1 (Today)**
- [ ] Complete basic profile information
- [ ] Set business hours to 24/7
- [ ] Add all service areas
- [ ] Upload 10 best vehicle photos

### **Day 2**
- [ ] Upload 20 more photos (airport, routes, customers)
- [ ] Add 10 services with descriptions & links
- [ ] Pre-populate 10 Q&As

### **Day 3**
- [ ] Add remaining 10 Q&As
- [ ] Create first 3 GMB posts
- [ ] Contact last 20 customers for reviews
- [ ] Respond to any existing reviews

### **Day 4-5**
- [ ] Upload remaining photos (target 50 total)
- [ ] Create first video
- [ ] Add remaining services

### **Day 6-7**
- [ ] Monitor insights
- [ ] Respond to new reviews/questions
- [ ] Follow up with customers for reviews
- [ ] Plan next week's content

---

## 📞 **Contact for Review Requests**

**Message Template (Copy-Paste Ready):**

```
Hi [Customer Name]! 👋

Thank you for choosing GreenEVCabs for your [Pune to Mumbai Airport / Other Route] journey! We hope you enjoyed the comfortable, eco-friendly ride in our electric cab.

Your feedback means a lot to us! Would you mind taking 1 minute to share your experience on Google?

⭐ Leave a Review: [Insert GMB Review Link]

As a thank you, enjoy ₹200 OFF your next booking! 🎁

Use code: REVIEW200

Book again: https://greenevcabs.com
WhatsApp: +91 74982 64215

Thank you for going green with us! 🌱⚡

- Team GreenEVCabs
```

---

## 🎁 **Bonus: Review Generation Tactics**

1. **Immediate Post-Ride:**
   - Send review link within 15 minutes of drop
   - Customer satisfaction is highest right after trip

2. **Driver Incentive:**
   - ₹100 bonus for each 5-star review collected
   - Trains drivers to provide excellent service

3. **Happy Customer List:**
   - Maintain list of customers who verbally praised service
   - Priority targets for review requests

4. **Event-Based:**
   - After particularly good experiences (early morning, delayed flights handled well)
   - "We noticed your flight was delayed 2 hours, hope we made your journey smooth! Mind sharing a review?"

5. **Seasonal Campaigns:**
   - Diwali: "Share your festival travel experience"
   - New Year: "Start the year by supporting local green business"

---

**This checklist ensures GreenEVCabs dominates local search for electric cab services and becomes the #1 choice for Pune-Mumbai airport transfers!** 🚀🌱⚡
